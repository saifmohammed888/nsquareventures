...
export default function Admin(){
  ...
  const stats = useMemo(() => [['Projects', content.projects.length], ['Blogs', content.articles.length], ['Media', (content.media || []).length], ['Site images', Object.values(content.site || {}).filter(Boolean).length]], [content]);
    ...
    <>
      ...
      <main className="min-h-screen bg-neutral-50 text-black">
        ...
        <div className="mx-auto grid max-w-7xl gap-5 px-5 py-6 lg:grid-cols-[240px_1fr]">
          ...
          {tab === 'images' && <ImagesTab imageOptions={mediaOptions} content={content} setContent={setContent} updateSite={updateSite} uploading={uploading} mediaDraft={mediaDraft} setMediaDraft={setMediaDraft} uploadMedia={uploadMedia} sync={sync} />}
...
function ImagesTab({ content, setContent, updateSite, uploading, imageOptions, mediaDraft, setMediaDraft, uploadMedia, sync }){
  ...
  return <section className="grid gap-5"><Editor title="Media library"><form onSubmit={uploadMedia} className="grid gap-4 border border-neutral-200 bg-neutral-50 p-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end"><Field label="Image name"><TextInput value={mediaDraft.name} onChange={event => setMediaDraft(current => ({ ...current, name: event.target.value }))} placeholder="Homepage hero, Nadeem portrait..." /></Field><Field label="Description"><TextInput value={mediaDraft.description} onChange={event => setMediaDraft(current => ({ ...current, description: event.target.value }))} placeholder="Where this image is useful" /></Field><label className="grid gap-2 text-xs font-semibold uppercase tracking-wide text-neutral-600"><span>File</span><input type="file" accept="image/*" onChange={event => setMediaDraft(current => ({ ...current, file: event.target.files?.[0] || null }))} className="block h-11 text-sm file:mr-3 file:h-11 file:border-0 file:bg-black file:px-4 file:text-sm file:font-semibold file:text-white" /></label><div className="lg:col-span-3"><Button type="submit" disabled={uploading === 'media'}>{uploading === 'media' ? 'Uploading...' : 'Upload to media library'}</Button><p className="mt-2 text-xs text-neutral-500">Upload once here, then reuse the image in projects, blogs, and site image slots.</p></div></form><div className="grid gap-2">{(content.media || []).map(item => <div key={item.id || item.url} className="grid grid-cols-[48px_1fr_auto] items-center gap-3 border border-neutral-200 bg-white p-2"><img src={item.url} alt="" className="h-12 w-12 bg-neutral-100 object-cover" /><div className="min-w-0"><b className="block truncate text-sm">{item.name || 'Untitled image'}</b><p className="truncate text-xs text-neutral-500">{item.description || item.url}</p></div><Button tone="danger" className="h-9 px-3 text-xs" onClick={() => removeMedia(item.id)}>Remove</Button></div>)}{!(content.media || []).length && <p className="border border-dashed border-neutral-300 p-6 text-sm text-neutral-500">No named media yet. Upload an image above to start the reusable library.</p>}</div></Editor><Editor title="Assign site images"><div className="grid gap-3 md:grid-cols-2">{siteImageFields.map(([key, title, help]) => <div key={key} className="grid grid-cols-[52px_1fr] gap-3 border border-neutral-200 bg-white p-3"><img src={content.site[key] || ''} alt="" className="h-12 w-12 bg-neutral-100 object-cover" /><div className="min-w-0"><h3 className="text-sm font-semibold">{title}</h3><p className="mb-3 mt-1 line-clamp-2 text-xs text-neutral-500">{help}</p><ImageField label="Image" value={content.site[key] || ''} onChange={value => updateSite(key, value)} imageOptions={imageOptions} /></div></div>)}</div></Editor></section>;
}
